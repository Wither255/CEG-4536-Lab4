...
int main()
{
    ...
    // Allocate input matrix on heap (Accomodate 4096x4096 matrix)
...
cudaError_t naiveReduction(int* h_result, const int* h_in, int size)
{
    ...
    int* intermediate = (int*)malloc(gridSize * sizeof(int));
    if (intermediate == NULL) {
        fprintf(stderr, "Failed to allocate intermediate!");
        goto Error;
    }

    cudaStatus = cudaMemcpy(intermediate, dev_data, gridSize * sizeof(int), cudaMemcpyDeviceToHost);
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMemcpy to host failed!");
        free(intermediate);
        goto Error;
    ...
    // Sum all block results on host
    int final_sum = 0;
    for (int i = 0; i < gridSize; i++) {
        final_sum += intermediate[i];
    }

    h_result[0] = final_sum;
